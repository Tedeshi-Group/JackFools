...
import ( // Начинаем блок импортов.
	...
	"context"   // Используем context.Context для интерфейса BotnetManagerAPI.
...
func handleEvent(event *GameEvent, manager *BotnetManager) error { // Функция обработки события.
	...
	if handler.Detect(event) { // Если обработчик может обработать событие.
		log.Printf("coordinator: using modular handler for game %s", event.GameTag) // Логируем использование модульного обработчика.
		return handler.Handle(event, &managerAdapter{manager: manager}) // Вызываем обработчик.
...
func (a *managerAdapter) SendCommand(cmd games.ClientCommand) {
	a.manager.mu.RLock()
	clientCount := len(a.manager.clients)
	...
	for i := 0; i < clientCount; i++ {
		a.manager.commandChan <- ClientCommand{
			Type:    cmd.Type,
			EventID: cmd.EventID,
			Answer:  cmd.Answer,
			Payload: cmd.Payload,
...
func (a *managerAdapter) GetAnswerDB(gameTag string) *games.AnswerDatabase {
	a.manager.mu.RLock()
	defer a.manager.mu.RUnlock()

	var db *AnswerDatabase
	if strings.Contains(gameTag, "-tjsp") {
		db = a.manager.answerDBTJSP
	}
	return db
}

// GetFinalRoundDB возвращает базу ответов финального раунда.
func (a *managerAdapter) GetFinalRoundDB(gameTag string) *games.AnswerDatabase {
	a.manager.mu.RLock()
	defer a.manager.mu.RUnlock()

	var db *AnswerDatabase
	if strings.Contains(gameTag, "-tjsp") {
		db = a.manager.finalRoundDBTJSP
	}
...
func (a *managerAdapter) GetContext() context.Context { // Функция получения контекста.
...
func sendPollPositionAnswersToClients(event *GameEvent, percentage int, questionInfo *PollPositionQuestionInfo, manager *BotnetManager) error { // Функция отправки ответов всем клиентам.
	...
	// BUG FIX: The original code had the votes swapped — firstChoiceCount bots sent vote="1"
	// and secondChoiceCount bots sent vote="0", which is the opposite of what the user intended.
	// The user enters a percentage for the first choice (vote="0"), so firstChoiceCount bots
	// must send vote="0" and secondChoiceCount bots must send vote="1".

	// Отправляем команды для первого варианта (vote="0"): firstChoiceCount ботов голосуют за первый вариант.
	for i := 0; i < firstChoiceCount; i++ { // Проходим по количеству ботов для первого варианта.
		cmd := ClientCommand{ // Создаём команду.
			Type:    "answer",                     // Устанавливаем тип команды.
			EventID: event.EventID,                // Устанавливаем ID события.
			Answer:  "0",                          // Отправляем vote="0" — первый вариант ответа.
			Payload: make(map[string]interface{}), // Инициализируем payload.
		} // Конец создания команды.
		...
		cmd.Payload["gameTag"] = event.GameTag                   // Добавляем тег игры.
		cmd.Payload["eventType"] = event.Type                    // Добавляем тип события.
		cmd.Payload["vote"] = "0"                                // Голосуем за первый вариант.
	...
	// Отправляем команды для второго варианта (vote="1"): secondChoiceCount ботов голосуют за второй вариант.
	for i := 0; i < secondChoiceCount; i++ { // Проходим по количеству ботов для второго варианта.
		cmd := ClientCommand{ // Создаём команду.
			Type:    "answer",                     // Устанавливаем тип команды.
			EventID: event.EventID,                // Устанавливаем ID события.
			Answer:  "1",                          // Отправляем vote="1" — второй вариант ответа.
			Payload: make(map[string]interface{}), // Инициализируем payload.
		} // Конец создания команды.
		...
		cmd.Payload["gameTag"] = event.GameTag                   // Добавляем тег игры.
		cmd.Payload["eventType"] = event.Type                    // Добавляем тип события.
		cmd.Payload["vote"] = "1"                                // Голосуем за второй вариант.
		cmd.Payload["opcode"] = "audience/count-group/increment" // Добавляем opcode.
		cmd.Payload["name"] = "Poll Position Vote"               // Добавляем name группы подсчёта.
...
func handleQuiplash2Event(event *GameEvent, manager *BotnetManager) error { // Функция обработки событий Quiplash 2.
	...
	answer, found := getAutoAnswer(event, nil) // Получаем автоматический ответ.
